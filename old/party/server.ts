    ...
    try {
      ...
      if (parsed.type === 'CLIENT_INFO') {
        ...
        if (clientType === 'web') {
          ...
          // Notifier tous les clients du changement de host
      ...
      if (parsed.type === 'START_GAME') {
        ...
        if (clientId !== this.hostId) {
          console.warn(`[Server] Tentative de démarrage du jeu par un non-host: ${clientId}`);
          return;
        }

        // Vérifier que le jeu n'est pas déjà démarré
        if (this.isGameStarted) {
          console.warn('[Server] Le jeu est déjà démarré');
          return;
        }
        ...
        if (players.length === 0) {
          console.warn('[Server] Aucun joueur dans la room (seul le host est présent) - Le jeu nécessite au moins un joueur mobile');
          return;
        }
