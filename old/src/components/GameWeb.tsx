...
export default function GameWeb({ roomName }: GameWebProps) {
  ...
  const handleStartGame = () => {
    console.log('[GameWeb] handleStartGame appelé, isHost:', isHost, 'isConnected:', isConnected);
    if (isHost) {
      console.log('[GameWeb] Démarrage du jeu...');
      startGameServer();
      // Le timer sera déclenché via le callback onGameStart quand le serveur diffusera START_GAME
    } else {
      console.log('[GameWeb] Vous n\'êtes pas le host, impossible de démarrer le jeu');
    }
  };
